...
<script >
	import { food } from '$js/sortdata.js';
...
<section class="container mx-auto p-6 font-mono">
	<div class="w-full mb-8 overflow-hidden">
	  <div class="w-full">
		...
		<Datatable settings={settings} data={food}>
		  <thead class="bg-dark-darker text-white">
			<tr class="text-lg font-semibold tracking-wide text-left text-gray-900 bg-gray-100 uppercase border border-dark-dark">
			  ...
			  <th class="sortable cursor-pointer desc px-4 py-4" data-key="(row) => row.data.result.name">Output<span></span></th>
			</tr>
		  </thead>
		  <tbody class="text-white">
			...
			<tr class="text-gray-700 {i % 2 == 0 ? 'bg-dark' : 'bg-dark-light'}">
			  ...
			  {/if}
			  <td class="px-4 py-3 text-ms font-semibold border hover:bg-dark-darker transition border-dark-dark">
				<a rel="prefetch" href="/{product.data.result.name.replace(/ /g, '-')}">
					<div class="flex items-center text-sm">
						<div class="relative w-16 h-16 mr-3 md:block">
							<img class="object-cover w-full h-full" src="/images/icons/{product.data.result.icon}" alt="" loading="lazy" />
						</div>
						<div>
